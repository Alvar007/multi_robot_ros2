...
def generate_launch_description():
    ...
    world_file_name = 'turtlebot3_hetero_robots.world'
    ...
    robot_desc_path2 = os.path.join(get_package_share_directory("uned_khepera_description"), "urdf", "khepera.urdf")
    ...
    nav2_yaml_1 = os.path.join(get_package_share_directory(
        'localization_server'), 'config', 'khepera01_amcl_config.yaml')
    map_file = os.path.join(get_package_share_directory(
        'map_server'), 'config', 'turtlebot_area.yaml')   

    rviz_config_dir = os.path.join(get_package_share_directory('localization_server'), 'rviz', 'multi-hetero.rviz')
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            executable='robot_state_publisher',
            name='robot_state_publisher',
            parameters=[{'use_sim_time': use_sim_time, 'robot_description': Command(['xacro ', robot_desc_path2])}],
            output="screen"
        ),
        ...
        Node(
            ...
            parameters=[{'use_sim_time': True},
                        {'autostart': True},
                        {'bond_timeout':0.0},
                        {'node_names': ['map_server', 'tb3_0/amcl', 'amcl', 'tb3_0/planner_server',
